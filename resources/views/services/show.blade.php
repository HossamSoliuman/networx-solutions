@php
    $serviceItems = $service->localizedServiceItems();
    $reasons = $service->localizedReasons();
    $serviceName = $service->localizedName();
    $serviceExcerpt = $service->localizedExcerpt();
    $serviceStatement = $service->localizedStatement();
@endphp

@extends('layouts.public')

@section('title', $serviceName)
@section('description', $serviceExcerpt)
...
@section('content')
        ...
        <div class="mx-auto flex min-h-[31rem] max-w-[90rem] items-center px-5 py-12 sm:min-h-[34rem] sm:px-8 lg:min-h-[36rem] lg:px-12">
            <div class="w-full min-w-0 max-w-3xl">
                <nav class="site-reveal flex max-w-full items-center gap-2 overflow-hidden whitespace-nowrap text-xs font-bold uppercase tracking-[0.12em] text-blue-200/80"
                    aria-label="{{ __('public.accessibility.breadcrumb') }}">
                    <a href="{{ route('home') }}" class="transition-colors hover:text-white">{{ __('public.navigation.home') }}</a>
                    <span aria-hidden="true">/</span>
                    <a href="{{ route('services.index') }}" class="transition-colors hover:text-white">{{ __('public.navigation.services') }}</a>
                    <span aria-hidden="true">/</span>
    ...
    <section id="service-scope" class="relative overflow-hidden bg-canvas py-14 sm:py-18 lg:py-20">
        <div class="bg-reference-dots absolute end-0 top-0 h-80 w-80 opacity-60"></div>
                    ...
                    <div class="mt-6 grid gap-3 md:grid-cols-2" data-reveal-group>
                        @foreach ($serviceItems as $item)
                            <article class="group flex min-w-0 gap-4 rounded-2xl border border-blue-100 bg-white p-4 shadow-[0_12px_35px_-30px_rgba(10,22,40,0.7)] transition duration-200 hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-[0_18px_45px_-28px_rgba(56,189,248,0.5)]"
                                ...
                                <div class="min-w-0">
                                    <h3 class="font-display text-base font-bold leading-5 text-navy-950"><bdi>{{ $item['title'] }}</bdi></h3>
                                    <p class="mixed-direction mt-1.5 break-words text-sm leading-5 text-slate-600">{{ $item['description'] }}</p>
                                </div>
                            </article>
                        @endforeach
                    </div>
                ...
                <aside class="grid gap-5 xl:sticky xl:top-28" data-reveal>
                    <div class="overflow-hidden rounded-[1.75rem] bg-navy-950 text-white shadow-[0_24px_60px_-32px_rgba(10,22,40,0.8)]">
                        <div class="flex items-center gap-4 bg-linear-to-r from-brand-700 to-brand-500 px-6 py-5">
                            ...
                            <h2 class="font-display text-2xl font-bold">{{ __('public.service.why_choose_us') }}</h2>
                        </div>
                        ...
                        <ol class="relative grid gap-1 p-5 sm:p-6">
                            @foreach ($reasons as $reason)
                                <li class="group relative flex gap-4 py-3.5 first:pt-1 last:pb-1">
                                    @unless ($loop->last)
                                        <span class="absolute bottom-0 start-5 top-12 w-px bg-blue-700/60"></span>
                                    @endunless
                                    ...
                                    <div>
                                        <h3 class="font-display text-base font-bold text-white"><bdi>{{ $reason['title'] }}</bdi></h3>
                                        <p class="mixed-direction mt-1 text-sm leading-5 text-blue-100/75">{{ $reason['description'] }}</p>
                                    </div>
                                </li>
                            @endforeach
                        </ol>
                    ...
                    <div class="rounded-[1.5rem] border border-blue-100 bg-white p-6">
                        <p dir="auto" class="font-display text-xl font-bold leading-7 text-navy-950">{{ $service->localizedDescription() }}</p>
                        <a href="{{ route('contact', ['service' => $service->slug]) }}" data-modal-open="contact-modal" data-contact-service-id="{{ $service->id }}" class="button-primary mt-5 w-full">
                            {{ __('public.service.talk_to_specialist') }}
        ...
        <section class="bg-white py-8 sm:py-10">
            <div class="mx-auto max-w-[90rem] px-5 sm:px-8 lg:px-12">
                <div class="relative overflow-hidden rounded-[1.75rem] bg-linear-to-r from-navy-950 via-brand-800 to-brand-600 px-6 py-8 text-white sm:px-10 lg:flex lg:items-center lg:justify-between lg:gap-10 lg:px-12">
                    <div class="bg-reference-dots absolute end-0 top-0 h-full w-80 opacity-20"></div>
                    <div class="relative flex items-center gap-5">
                        <span class="flex size-14 shrink-0 items-center justify-center rounded-full border-2 border-white/80 bg-white/10">
                            <x-icon :name="$service->icon" class="size-7" />
                        </span>
                        <p class="max-w-3xl font-display text-2xl font-bold leading-tight sm:text-3xl">{{ $serviceStatement }}</p>
                    </div>
                    <a href="{{ route('contact', ['service' => $service->slug]) }}" data-modal-open="contact-modal" data-contact-service-id="{{ $service->id }}" class="button-light relative mt-6 shrink-0 lg:mt-0">
                        {{ __('public.service.start_conversation') }}
        ...
        <section class="border-t border-blue-100 bg-white py-14 sm:py-18">
            <div class="mx-auto max-w-[90rem] px-5 sm:px-8 lg:px-12">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p class="section-kicker">{{ __('public.service.more_services') }}</p>
                        <h2 class="mt-4 font-display text-3xl font-bold tracking-[-0.03em] text-navy-950 sm:text-4xl">{{ __('public.service.more_services_title') }}</h2>
                    </div>
                    <a href="{{ route('services.index') }}" class="text-link">{{ __('public.service.view_all') }} <x-icon name="arrow-left" class="size-4 rotate-180 rtl:rotate-0" /></a>
                </div>
