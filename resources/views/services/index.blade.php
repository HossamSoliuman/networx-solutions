...
@section('title', __('public.services.page_title'))
@section('description', $site['services_intro'])

@section('content')
    <section class="relative isolate overflow-hidden bg-navy-950 text-white">
        <img src="{{ asset('images/site/networking-banner.webp') }}" alt="{{ __('public.services.image_alt') }}"
        ...
        <div class="mx-auto grid min-h-[31rem] max-w-[90rem] items-center gap-10 px-5 py-12 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-12">
            <div class="max-w-3xl">
                <p class="site-reveal section-kicker text-blue-200">{{ __('public.services.eyebrow') }}</p>
                @php($headline = __('public.services.headline'))
                ...
                <div class="grid grid-cols-2 gap-3">
                    @foreach ($services as $service)
                        <a href="{{ route('services.show', $service) }}" class="group flex min-h-24 flex-col justify-between rounded-xl bg-white/7 p-4 ring-1 ring-white/10 transition hover:bg-brand-600/25 hover:ring-brand-300/50">
                            ...
                            <bdi class="mt-3 font-display text-sm font-bold leading-4 text-white">{{ $service->localizedName() }}</bdi>
                        </a>
                    @endforeach
                </div>
            </div>
    ...
    <section class="relative overflow-hidden bg-canvas py-14 sm:py-18 lg:py-20">
        <div class="bg-reference-dots absolute end-0 top-0 h-80 w-80 opacity-50"></div>
        <div class="relative mx-auto max-w-[90rem] px-5 sm:px-8 lg:px-12">
            <div class="grid gap-5 border-b border-blue-200 pb-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
                <div>
                    <p class="section-kicker">{{ __('public.services.starting_point') }}</p>
                    <p class="mt-4 max-w-md text-sm leading-6 text-slate-600">{{ __('public.services.starting_point_intro') }}</p>
                </div>
                <h2 class="font-display text-3xl font-bold leading-[1.02] tracking-[-0.035em] text-navy-950 sm:text-4xl lg:text-5xl">{{ __('public.services.expertise_title') }}</h2>
            </div>
    ...
    <section class="bg-white py-10 sm:py-12">
        <div class="mx-auto max-w-[90rem] px-5 sm:px-8 lg:px-12">
            <div class="rounded-[1.75rem] bg-linear-to-r from-navy-950 via-brand-800 to-brand-600 px-6 py-8 text-white sm:px-10 lg:flex lg:items-center lg:justify-between lg:gap-10 lg:px-12">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.16em] text-blue-200">{{ __('public.services.partner') }}</p>
                    <h2 class="mt-3 font-display text-3xl font-bold tracking-[-0.03em]">{{ __('public.services.cta_title') }}</h2>
                </div>
                <a href="{{ route('contact') }}" data-modal-open="contact-modal" class="button-light mt-6 shrink-0 lg:mt-0">{{ __('public.services.contact') }} <x-icon name="arrow-left" class="size-4 rotate-180 rtl:rotate-0" /></a>
            </div>
        </div>
    </section>
@endsection
