...
<dialog id="contact-modal" data-contact-modal @if ($errors->any()) data-open-on-load @endif
    ...
    <div class="relative overflow-hidden border-b border-slate-200 bg-navy-950 px-5 py-5 text-white sm:px-7">
        ...
        <div class="relative flex items-start justify-between gap-5">
            <div>
                <p class="technical-label text-brand-200">{{ __('public.contact.start_conversation') }}</p>
                <h2 id="contact-modal-title" class="mt-1.5 font-display text-2xl font-bold tracking-[-0.035em] sm:text-3xl">{{ __('public.contact.how_can_we_help') }}</h2>
            </div>
            <button type="button" data-modal-close
                ...
                aria-label="{{ __('public.contact.close_form') }}">
    ...
    <form method="POST" action="{{ route('contact.store') }}" class="grid gap-x-4 gap-y-3 p-5 sm:grid-cols-2 sm:p-7 lg:grid-cols-3"
        ...
        <div class="min-w-0">
            <x-form.label for="modal_email">{{ __('public.contact.work_email') }} <span class="text-brand-700" aria-hidden="true">*</span></x-form.label>
            <x-form.input id="modal_email" name="email" type="email" :value="old('email')" required autocomplete="email"
                :placeholder="__('public.contact.email_placeholder')" dir="ltr" class="mt-1 h-10 bg-white text-left"
        ...
        <div class="min-w-0">
            <x-form.label for="modal_phone_local">{{ __('public.contact.phone') }} <span class="text-brand-700" aria-hidden="true">*</span></x-form.label>
            ...
            <x-form.input id="modal_phone_local" name="phone_local" type="tel" :value="old('phone_local')" required autocomplete="tel-national" inputmode="tel"
                placeholder="10 664 055 70" dir="ltr" class="mt-1 h-10 bg-white text-left"
        ...
        <div class="min-w-0">
            <x-form.label for="modal_service_id">{{ __('public.contact.service') }}</x-form.label>
            <x-form.select id="modal_service_id" name="service_id" class="mt-1 h-10 bg-white py-1.5" data-contact-service-select
                ...
                @foreach ($services as $service)
                    <option value="{{ $service->id }}" @selected((string) old('service_id') === (string) $service->id)>{{ $service->localizedName() }}</option>
                @endforeach
            </x-form.select>
        ...
        <div class="absolute -start-[9999px]" aria-hidden="true">
            <label for="modal_company_fax">{{ __('public.contact.fax') }}</label>
